...
class QuoridorEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # first 4 moves will be the pawn moves ( 0 - up 1 - down 2 - right 3 - left), the other ones will be considered the wall places in this order:
    ...
    def enemys_move(self):
        ...
        if okRemakeLee == True:
            ...
            while len(q) != 0:
                [i, j] = q.pop(0)
                if i > 0:
                    if self.lee_matrix[i - 1][j] == 0 and (self.lee_matrix[i - 2][j] == 0 or self.lee_matrix[i - 2][j] > self.lee_matrix[i][j] + 1):
                    ...
                    if j > 0:
                        if self.lee_matrix[i][j - 1] == 0 and (self.lee_matrix[i][j - 2] == 0  or self.lee_matrix[i][j - 2] > self.lee_matrix[i][j] + 1):
            ...
            if min_pos == -1:
                ...
                if self.distance_lee != -1:
                    if self.distance_lee < min:
                        self.reward += 40 * (min - self.distance_lee)    
    ...
    def is_good_move(self, initial_x, initial_y):
        # implement lees algorithm

        lee_matrix = [[0 for _ in range(17)] for _ in range(17)]
        
        for i in range(len(self.wallPlaces)):
            for j in range(len(self.wallPlaces[0])):
                if self.wallPlaces[i][j] == 1:
                    lee_matrix[i * 2 + 1][j * 2 + 1] = -1
                    lee_matrix[i * 2 + 1][j * 2 + 0] = -1
                    lee_matrix[i * 2 + 1][j * 2 + 2] = -1
                elif self.wallPlaces[i][j] == 2:
                    lee_matrix[i * 2 + 1][j * 2 + 1] = -1
                    lee_matrix[i * 2 + 0][j * 2 + 1] = -1
                    lee_matrix[i * 2 + 2][j * 2 + 1] = -1
        
        q = []
        q.append([initial_x * 2, initial_y * 2])
        lee_matrix[initial_x * 2][initial_y * 2] = 1
        while len(q) != 0:
            [i, j] = q.pop(0)
            if i > 0:
                if lee_matrix[i - 1][j] == 0 and (lee_matrix[i - 2][j] == 0 or lee_matrix[i - 2][j] > lee_matrix[i][j] + 1):
                    lee_matrix[i - 2][j] = lee_matrix[i][j] + 1
                    q.append([i - 2, j])
            ...
            if j > 0:
                if lee_matrix[i][j - 1] == 0 and (lee_matrix[i][j - 2] == 0  or lee_matrix[i][j - 2] > lee_matrix[i][j] + 1):
                    lee_matrix[i][j - 2] = lee_matrix[i][j] + 1
                    q.append([i, j - 2])
        ...
        min_pos = -1
        for i in range(16):
            if min > lee_matrix[0][i] and lee_matrix[0][i] != -1 and lee_matrix[0][i] != 0:
                min = lee_matrix[0][i]
                min_pos = i

        if min_pos == -1:
            print("Oh no")
            for el in lee_matrix:
                print(el)
            print("Oh no")
        else:
            ct = min
            ...
            for el in lee_matrix:
                print(el)
            if self.positionAIPlayer[0] == i // 2 and self.positionAIPlayer[1] == j // 2:
                self.reward += 60

    def check_pawn_action(self, action):
        # move up
        initial_x, initial_y = self.positionAIPlayer[0], self.positionAIPlayer[1]
        if action == 0:
            ...
            self.is_good_move(initial_x, initial_y)
            return True
        # move down
        if action == 1:
    ...
    def step(self, action):
        ...
        print(self.positionLeePlayer, self.positionAIPlayer)
